...
## SELECTIONS -- set here what model, stages, and features to use
...
excluded_features = ['person_prefix', 'night_file', 'night_date', 'stage_group', 'group', 'minutes_of_data', 'n_overlap_epochs', 'minutes_overlap', 'minutes_total_hypnogram', 'TIB_minutes', 'hr_spike_times'] # The real frequency features have different names, so therefore these are excluded
...
plt.xlabel("Predicted")
plt.ylabel("True")
# Update title depending on features
...
# Compute relative (percentage) importances 
percent_importance = (importance_means / np.sum(importance_means)) * 100

# Plot top 15 most important features 
top_n = 15
top_idx = sorted_idx[:top_n]
plt.figure(figsize=(8,5))
plt.bar(np.array(feature_columns)[top_idx], percent_importance[top_idx])
...
for idx in top_idx:
    print(f"{feature_columns[idx]}: {percent_importance[idx]:.1f}%")
